...
def city_activity(request):
    ...
    Usa el patrón blob de Timescale con agregados precalculados.
    ...
    try:
        ...
        # Parse fechas YYYY-MM-DD y convertir a microsegundos
        try:
            start_date = datetime.strptime(start_str, '%Y-%m-%d')
            end_date = datetime.strptime(end_str, '%Y-%m-%d')
            
            # Convertir a microsegundos (como usa Timescale)
            start_ts = int(start_date.timestamp() * 1000000)
            end_ts = int(end_date.timestamp() * 1000000) + (86400 * 1000000)  # +1 día
        except ValueError:
            return JsonResponse(
        ...
        for location in Location.objects.all():
            ...
            # Filtrar datos por medición, estaciones y rango de tiempo (en microsegundos)
            ...
            # Calcular agregados usando los campos precalculados del patrón blob
